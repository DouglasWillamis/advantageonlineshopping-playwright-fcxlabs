...
export class CreateAccountPage extends GlobalPage {
    ...
    constructor(page: Page) {
        ...
        this.createAccountTitle = this.page.getByRole('heading', { name: 'CREATE ACCOUNT' })
        ...
        this.passwordInput = this.page.locator('input[name="passwordRegisterPage"]')
        this.confirmPasswordInput = this.page.locator('input[name="confirm_passwordRegisterPage"]')
    ...
    async verifyRequiredInputs() {
        ...
        await this.page.keyboard.press('Tab')
        await this.page.keyboard.press('Tab')
        await expect(this.page.getByText("Username field is required", {exact: true})).toBeVisible()
        await expect(this.page.getByText("Email field is required", {exact: true})).toBeVisible()
        await expect(this.page.getByText("Password field is required", {exact: true})).toBeVisible()
        await expect(this.page.getByText("Confirm password field is required", {exact: true})).toBeVisible()
    }

    async fillFormAndSubmitAndGoToCheckoutLoggedIn(user: User): Promise<CheckoutLoggedInPage> {
